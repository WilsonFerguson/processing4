...
public class PApplet implements PConstants {
     ...
     *
     * @webref image:pixels
     * @webBrief Array containing the values for all the pixels in the display
     *           window
     ...
     *
     * @webref input:mouse
     * @webBrief The system variable that always contains the current horizontal
     *           coordinate of the mouse
     * @see PApplet#mouseY
     * @see PApplet#pmouseX
     * @see PApplet#pmouseY
     ...
     *
     * @webref input:mouse
     * @webBrief The system variable that always contains the current vertical
     *           coordinate of the mouse
     * @see PApplet#mouseX
     * @see PApplet#pmouseX
     * @see PApplet#pmouseY
     ...
     * updated multiple times per frame, then something like <b>line(pmouseX,
     * pmouseY,
     ...
     *             from future releases of Processing because it cannot be used
     *             consistently
    ...
    /**
     * The <b>mousePressed</b> variable stores whether a mouse button has been
     * pressed.
     * The <b>mouseButton</b> variable (see the related reference entry) can be used
     * to
     ...
     *           recent key on the keyboard that was used (either pressed or
     *           released)
     * @see PApplet#keyCode
     ...
     * <a href=
     * "https://docs.oracle.com/javase/8/docs/api/java/awt/event/KeyEvent.html">KeyEvent</a>
     * reference.
     * <br />
     * <br />
     * There are issues with how <b>keyCode</b> behaves across different
     ...
     *
     * @webref input:keyboard
     * @webBrief Used to detect special keys such as the UP, DOWN, LEFT, RIGHT arrow
     ...
     *
     * @webref environment
     * @webBrief Used when absolutely necessary to define the parameters to
     *           <b>size()</b>
     *           with a variable
    ...
    private void smoothWarning(String method) {
        ...
        if (external) {
            PGraphics.showWarning("When run from the PDE, %s() is automatically moved from setup() to settings()",
                    method);
        }
    }
     ...
     * <ul>
     * In addition, the new (for 2.0) <tt>processing.event</tt> classes are passed
     * to
     ...
     * to define initial environment properties such as screen size and to load
     * media
     ...
     * <br />
     * There can only be one <b>draw()</b> function for each sketch, and
     * <b>draw()</b>
     * must exist if you want the code to run continuously, or to process events
     * such
     ...
     *
     * @webref structure
     * @webBrief Called directly after <b>setup()</b> and continuously executes the
     *           lines
    ...
    /**
     * @param display the screen to run the sketch on (1, 2, 3, etc. or on multiple
     *                screens using SPAN)
     */
     ...
     *
     * @webref structure
     * @webBrief Stops Processing from continuously executing the code within
     *           <b>draw()</b>
     * @usage web_application
     * @see PApplet#loop()
     * @see PApplet#redraw()
     ...
     * <p/>
     * The screen only updates when the end of <b>draw()</b> is reached, so
     * <b>delay()</b>
     * cannot be used to slow down drawing. For instance, you cannot use
     * <b>delay()</b>
     ...
     *
     * @webref environment
     * @webBrief The <b>delay()</b> function causes the program to halt for a
     *           specified time
    ...
    static public int exec(StringList stdout, StringList stderr, String... args) {
         ...
         * try {
         * saveStream(baos, out);
         * BufferedReader err2 = createReader(new
    ...
    static public int shell(StringList stdout, StringList stderr, String... args) {
        ...
        if (platform == WINDOWS) {
            ...
            // can't use -i or -l since the version of bash shipped with macOS does not
            // support this together with -c
            ...
            argList.append(
                    "if [ -f ~/.bash_profile ]; then . ~/.bash_profile >/dev/null 2>&1; elif [ -f ~/.bash_profile ]; then . ~/.bash_profile >/dev/null 2>&1; elif [ -f ~/.profile ]; then ~/.profile >/dev/null 2>&1; fi;");
        }
        for (String arg : args) {
            argList.append(arg);
        }
     ...
     * something
     * other than actually terminate the JVM. This gives them a chance to do
     * whatever
     ...
     * @see PApplet#frameCount
     * @param filename
     *                 any sequence of letters or numbers that ends with either
     *                 ".tif",
     *                 ".tga", ".jpg", or ".png"
     */
     ...
     *
     * @webref math:calculation
     * @webBrief Returns Euler's number <i>e</i> (2.71828...) raised to the power of
     *           the
     *           <b>value</b> parameter
     ...
     *
     * @webref math:calculation
     * @webBrief Calculates the closest int value that is greater than or equal to
     *           the
     ...
     *
     * @webref math:calculation
     * @webBrief Calculates the closest int value that is less than or equal to the
     *           value
     *           of the parameter
     ...
     * value. Therefore, <b>mag()</b> is a shortcut for writing <b>dist(0, 0, x,
     * y)</b>.
     *
     * @webref math:calculation
     ...
     * @param filename
     *                 name of file to load, can be .gif, .jpg, .tga, or a handful
     *                 of
     ...
     * @param filename
     *                  name of the file to load, can be .gif, .jpg, .tga, or a
     *                  handful of
     *                  other image types depending on your platform
     * @param extension
     ...
     * parameter can also be a URL to a file found online.<br />
     * <br />
     * All files loaded and saved by the Processing API use
     * UTF-8 encoding. If you need to load an XML file that's
     * not in UTF-8 format, see the <a href=
     * "http://processing.github.io/processing-javadocs/core/processing/data/XML.html">
     ...
     * <b>parseXML()</b>. Be sure to check if <b>null</b> is returned before
     * performing
     ...
     *
     * @webref input:files
     * @webBrief Reads the contents of a file or URL and creates a <b>Table</b>
     *           object
     *           with its values
     * @param filename
     *                 name of a file in the data folder or a URL.
     ...
     * 'callback' function. If the dialog is closed or canceled, <b>null</b> will be
     * sent
     ...
     *
     * @webref output:files
     * @webBrief Opens a platform-specific file chooser dialog to select a file for
     *           output
     ...
     *
     * @webref output:files
     * @webBrief Creates a new file in the sketch folder, and a <b>PrintWriter</b>
     *           object
     *           to write to it
     ...
     *
     * @webref input:files
     * @webBrief Reads the contents of a file or url and creates a <b>String</b>
     *           array of
     ...
     * @param dst
     *                    the destination array of the same data type as the source
     *                    array
     * @param dstPosition
     ...
     * @webBrief Removes whitespace characters from the beginning and end of a
     *           <b>String</b>
     ...
     *
     * @webref data:string_functions
     * @webBrief Combines an array of <b>Strings</b> into one <b>String</b>, each
     *           separated by the
    ...
    /**
     *
     * The <b>splitTokens()</b> function splits a <b>String</b> at one or many
     * character
     ...
     *
     * @webref data:string_functions
     * @webBrief The <b>splitTokens()</b> function splits a <b>String</b> at one or
     *           many
     *           character "tokens"
     * @param value
     ...
     * array. If there are no matches, a <b>null</b> value will be returned. If no
     * groups
     ...
     * @webBrief Converts a <b>byte</b>, <b>char</b>, <b>int</b>, or <b>color</b> to
     *           a <b>String</b> containing the
     ...
     *
     * @webref data:conversion
     * @webBrief Converts a <b>String</b> representation of a hexadecimal number to
     *           its
     *           equivalent integer value
     * @param value
     ...
     *
     * @webref data:conversion
     * @webBrief Converts an <b>int</b>, <b>byte</b>, <b>char</b>, or <b>color</b>
     *           to a
     ...
     * capped at 1. This is different from the behavior of <b>lerp()</b>, but
     * necessary
     * because otherwise numbers outside the range will produce strange and
     * unexpected colors.
     *
     * @webref color:creating & reading
     * @webBrief Calculates a <b>color</b> or <b>colors</b> between two
     *           <b>colors</b> at a specific
     *           increment
     * @usage web_application
     * @param c1
    ...
    static public void runSketch(final String[] args,
        ...
        int argIndex = 0;
        label: while (argIndex < args.length) {
        ...
        if (platform == WINDOWS) {
            ...
            if (uiScale != 0) {
                ...
                // } else {
                // System.err.println("Could not identify Windows DPI, not setting
                // sun.java2d.uiScale");
            }
        }
        ...
        if (platform == MACOS && !disableAWT) {
            try {
                ...
                Method method = thinkDifferent.getMethod("init", PApplet.class);
    ...
    /**
     * @nowebref
     *           Begin recording (echoing) commands to the specified PGraphics
     *           object.
     */
     ...
     *
     * @webref output:files
     * @webBrief Stops the recording process started by <b>beginRecord()</b> and
     *           closes
     *           the file
     ...
     *           (rather than
     *           generated by preproc.pl) for clarity and so that it doesn't echo
     *           the
     ...
     * counterclockwise.<br />
     * <br />
     * These functions can only be used within a
     * <b>beginShape()</b>/<b>endShape()</b>
     ...
     * in clockwise order, then for internal shapes, draw vertices
     * counterclockwise.<br />
     * <br />
     * These functions can only be used within a
     * <b>beginShape()</b>/<b>endShape()</b>
     ...
     *
     * @webref shape:vertex
     * @webBrief the companion to <b>beginShape()</b> and may only be called after
     *           <b>beginShape()</b>
     ...
     * PShape class are in the
     * <a href="http://processing.github.io/processing-javadocs/core/">Processing
     * Javadoc</a>.
     *
     * @webref shape
     ...
     *
     * @webref rendering
     * @webBrief Disables the clipping previously started by the <b>clip()</b>
     *           function
     */
    public void noClip() {
        if (recorder != null)
     ...
     * <br />
     * BLEND - linear interpolation of colors: <b>C = A*factor + B</b>. This is the
     * default.<br />
     * <br />
     * ADD - additive blending with white clip: <b>C = min(A*factor + B,
     * 255)</b><br />
     * <br />
     ...
     * <br />
     * LIGHTEST - only the lightest color succeeds: <b>C = max(A*factor,
     * B)</b><br />
     * <br />
     ...
     */
    public void point(PVector position) {
        if (recorder != null)
            recorder.point(position.x, position.y, position.z);
        g.point(position);
    }
     ...
     */
    public void line(PVector position1, PVector position2) {
        if (recorder != null)
            recorder.line(position1.x, position1.y, position1.z,
                    position2.x, position2.y, position2.z);
        g.line(position1, position2);
    }
    ...
    /**
     * @param position1 position of the first point
     * @param position2 position of the second point
     * @param position3 position of the third point
     */
    public void triangle(PVector position1, PVector position2, PVector position3) {
        if (recorder != null)
            recorder.triangle(position1.x, position1.y, position2.x, position2.y,
                    position3.x, position3.y);
        g.triangle(position1, position2, position3);
    }
    ...
    /**
     * @param position1 position of the first corner
     * @param position2 position of the second corner
     * @param position3 position of the third corner
     * @param position4 position of the fourth corner
     */
    public void quad(PVector position1, PVector position2,
            PVector position3, PVector position4) {
        if (recorder != null)
            recorder.quad(position1.x, position1.y, position2.x, position2.y,
                    position3.x, position3.y, position4.x, position4.y);
        g.quad(position1, position2, position3, position4);
    }
     ...
     */
    public void rect(PVector position, PVector extent) {
        if (recorder != null)
            recorder.rect(position.x, position.y, extent.x, extent.y);
        g.rect(position, extent);
    }
     ...
     * @param r        radii for all four corners
     */
    public void rect(PVector position, PVector extent, float r) {
        if (recorder != null)
            recorder.rect(position.x, position.y, extent.x, extent.y, r);
        g.rect(position, extent, r);
    }
    ...
    public void rect(PVector position, PVector extent,
            float tl, float tr, float br, float bl) {
        if (recorder != null)
            recorder.rect(position.x, position.y, extent.x, extent.y, tl, tr, br, bl);
        g.rect(position, extent, tl, tr, br, bl);
    }
     ...
     * @param extent   width and height of the square
     */
    public void square(PVector position, float extent) {
        if (recorder != null)
            recorder.square(position.x, position.y, extent);
        g.square(position, extent);
    }
     ...
     */
    public void ellipse(PVector position, float w, float h) {
        if (recorder != null)
            recorder.ellipse(position.x, position.y, w, h);
        g.ellipse(position, w, h);
    }

    /**
     * @param position position of the ellipse
     * @param extent   width and height of the ellipse
     */
    public void ellipse(PVector position, PVector extent) {
        if (recorder != null)
            recorder.ellipse(position.x, position.y, extent.x, extent.y);
        g.ellipse(position, extent);
    }
     ...
     * @param extent   width and height of the arc
     * @param start    angle to start the arc, specified in radians
     * @param stop     angle to stop the arc, specified in radians
     */
    public void arc(PVector position, PVector extent,
            float start, float stop) {
        if (recorder != null)
            recorder.arc(position.x, position.y, extent.x, extent.y, start, stop);
        g.arc(position, extent, start, stop);
    }
    ...
    public void arc(PVector position, PVector extent,
            float start, float stop, int mode) {
        if (recorder != null)
            recorder.arc(position.x, position.y, extent.x, extent.y, start, stop, mode);
        g.arc(position, extent, start, stop, mode);
    }
     ...
     * @param extent   width and height of the circle
     */
    public void circle(PVector position, float extent) {
        if (recorder != null)
            recorder.circle(position.x, position.y, extent);
        g.circle(position, extent);
    }
     ...
     * @webref shape:3d primitives
     * @webBrief Controls the detail used to render a sphere by adjusting the number
     *           of
     ...
     * This function is only useful when using the <b>P2D</b> or <b>P3D</b>
     * renderer;
     ...
     *
     * @webref transform
     * @webBrief Prints the current matrix to the Console (the text window at the
     *           bottom
     *           of Processing)
     * @see PGraphics#pushMatrix()
     * @see PGraphics#popMatrix()
     ...
     * the <b>camera()</b> function will be sufficient.<br />
     * <br />
     * The camera
     ...
     * to <b>gluLookAt()</b> in OpenGL, but it first clears the current camera
     * settings.
     *
     * @webref lights_camera:camera
     ...
     *
     * @webref lights_camera:camera
     * @webBrief Prints the current camera matrix to the Console (the text window at
     *           the
     *           bottom of Processing)
     * @see PGraphics#camera(float, float, float, float, float, float, float, float,
     *      float)
     ...
     *
     * @webref lights_camera:camera
     * @webBrief Sets a perspective projection applying foreshortening, making
     *           distant
     ...
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the X value
     *           for
     *           where it will appear on a (two-dimensional) screen
     ...
     *
     * @webref lights_camera:coordinates
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the Y value
     *           for
     *           where it will appear on a (two-dimensional) screen
     * @param x 3D x-coordinate to be mapped
     * @param y 3D y-coordinate to be mapped
     ...
     *
     * @webref lights_camera:coordinates
     * @webBrief Takes a three-dimensional X, Y, Z position and returns the Z value
     *           for
     *           where it will appear on a (two-dimensional) screen
     * @param x 3D x-coordinate to be mapped
     * @param y 3D y-coordinate to be mapped
     ...
     * in the style:
     * <b>fill()<b>, <b>stroke()</b>, <b>tint()</b>, <b>strokeWeight()</b>,
     * <b>strokeCap()</b>,<b>strokeJoin()</b>,
     * <b>imageMode()</b>, <b>rectMode()</b>, <b>ellipseMode()</b>,
     * <b>shapeMode()</b>, <b>colorMode()</b>,
     * <b>textAlign()</b>, <b>textFont()</b>, <b>textMode()</b>, <b>textSize()</b>,
     * <b>textLeading()</b>,
     * <b>emissive()</b>, <b>specular()</b>, <b>shininess()</b>, <b>ambient()</b>
     *
     * @webref structure
     * @webBrief Saves the current style settings and <b>popStyle()</b> restores the
     *           prior settings
     ...
     *
     * @webref lights_camera:material properties
     * @webBrief Sets the specular color of the materials used for shapes drawn to
     *           the
     ...
     *
     * @webref lights_camera:material properties
     * @webBrief Sets the emissive color of the material used for drawing shapes
     *           drawn to
     *           the screen
     * @usage web_application
     * @param rgb color to set
     ...
     * <b>directionalLight(128, 128, 128, 0, 0, -1)</b>, <b>lightFalloff(1, 0,
     * 0)</b>, and
     * <b>lightSpecular(0, 0, 0)</b>. Lights need to be included in the
     * <b>draw()</b> to
     ...
     *
     * @webref lights_camera:lights
     * @webBrief Sets the default ambient light, directional light, falloff, and
     *           specular
     *           values
     * @usage web_application
     ...
     * @param max2 range for the green or saturation depending on the current color
     *             mode
     ...
     * <br />
     * LIGHTEST - only the lightest colour succeeds: <b>C = max(A*factor,
     * B)</b><br />
     * <br />
     * DIFFERENCE - subtract colors from underlying image.<br />
     * <br />
     ...
     *
     * @webref image:pixels
     * @webBrief Copies a pixel or rectangle of pixels using different blending
     *           modes
     * @param src  an image variable referring to the source image
     * @param sx   X coordinate of the source's upper left corner
     * @param sy   Y coordinate of the source's upper left corner
